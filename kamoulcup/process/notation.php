...
/**
 * Calcule le score d'un joueur selon une nouvelle m�thode : score journ�e par journ�e
	...
	if ($prestations != NULL) {
		foreach($prestations as $presta) {
			...
			if (intval($presta['minutes']) >= $SCO_minTps && floatval($presta['score']) > 0)
			{
				...
				// Si on n'en avait pas d�j� assez, on l'ajoute de toute fa�on
				if(count($bestNotes) < $SCO_nbperfs) {
					$bestNotes[]=$prestaNote;
					sort($bestNotes);
				} else {
					// on a d�j� le nombre voulu de notes : si c'est une note sup�rieure au minimum, on la compte, sinon non.
					$minNote = $bestNotes[0];
			...
			// on calcule ici la partie du score d�pendante des notes.
...
function calculScorePrestation($db,$prestationId) {
	...
	if ($noteFF != NULL) {
		$compteurNote++;
		// noteFF est une note "� l'allemande": il faut la convertir.
