<?php
  ...
  function insertButeur($matchId,$buteurId,$passeurId,$isPenalty,$isProlongation,$domExt,$db) {
	...
	function updateButsPrestation($prestationId,$db) {
		...
		foreach($getButeursQuery as $bRow) {
			if ($bRow['penalty']) {
				...
				// Le penalty est le seul cas de figure o� l'on peut se faire une passe d�cisive � soit meme, donc pas de 'else' ici.
	...
	// Met � jour le flag "leader" de la talbe prestation pour enregistrer les leaders de chaque ligne du match concern�.
	...
	// Met � 1 le flag leader de la meilleure (ou des meilleures) prestation en param, et remet les autres � 0
	function setLeader($db,$prestationsArray) {
		global $SCO_minTps;
	// rep�rer la prestation la plus haute et mettre � jour son flag.
		if ($prestationsArray != NULL) {
			...
			foreach($prestationsArray as $prest) {
				...
				// si score inf�rieur, on ne fait rien
			}
			// Puis on met les flag leader � 1
