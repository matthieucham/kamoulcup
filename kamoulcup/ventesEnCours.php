...
foreach ($listPoulesQuery as $poule) {
	$periodeEnCours = getOpenPeriode(intval($poule['id']),TRUE);
	if ($periodeEnCours['draft']) {
	} else {
		...
		if (dirigeEkypDeLaPoule($poule['id'])){
			...
			$paAllowed = (($futurBudget > 0) && ($nbJoueurs < $EKY_nbmaxjoueurs) && ($nbPA < $EKY_nbPA) && ($periodeEnCours != NULL));
...
<div class='sous_titre'><?php echo $poule['nom']; ?></div>
	<?php
	if ($periodeEnCours != NULL && intval($periodeEnCours[0]['draft'])>0) {
		echo "<p>DRAFT EN COURS !</p>";
		if (dirigeEkypDeLaPoule($poule['id']))
		{
			echo "<p>Envoyez vos voeux avant le <b>{$periodeEnCours[0]['fin']}</b>.</p>";
			echo "» <a href=\"index.php?page=editDraft\">Enregistrer ou modifier ses voeux</a><br/>";
		}
	}
	else {
		if ($periodeEnCours != NULL)
		{
			echo "<p>Mises aux enchères autorisées du <b>{$periodeEnCours[0]['debut']} au {$periodeEnCours[0]['fin']}</b>.</p>";
			echo "<p>Les enchères durent <b>{$periodeEnCours[0]['delai_encheres']} heures</b> à partir de la mise en vente.</p>";
