...
function registerEngagement($inscriptionId,$joueurId,$montant,$journee,$redistrib, $nbFranchises, $mercatoId){
	...
	$db->query("insert into km_engagement(eng_inscription_id,eng_joueur_id,eng_salaire,eng_date_arrivee,eng_montant) values ({$inscriptionId},{$joueurId},(select scl_salaire from km_const_salaire_classe inner join km_join_joueur_salaire on scl_id=jjs_salaire_classe_id where jjs_journee_id={$journee} and jjs_joueur_id={$joueurId}), now(), {$montant})");
	// Finances
