...
function getCurrentMercato() {
    global $db;
    $merkatoQ="select mer_id,date_format(mer_date_ouverture,'%d/%m/%Y %H:%i') as dateOuverture,date_format(mer_date_fermeture,'%d/%m/%Y %H:%i') as dateFermeture from km_mercato where mer_date_ouverture<now() and mer_date_fermeture>now() and mer_processed=0 limit 1";
